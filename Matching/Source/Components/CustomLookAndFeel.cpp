                                          ...
                                          const float rotaryStartAngle, const float rotaryEndAngle, juce::Slider& slider)
{
    ...
    /*auto alpha = 0.1f + (float) slider.getValue() / (float)slider.getMaximum() * 0.9f;
    auto brightness = 0.4f + (float) slider.getValue() / (float)slider.getMaximum() * 0.6f;

    g.setColour (fill.withAlpha (alpha).brighter (brightness));*/
    g.setColour(fill.withAlpha(1.f).brighter(1.f));
