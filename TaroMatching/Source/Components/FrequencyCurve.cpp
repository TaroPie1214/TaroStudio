...
FrequencyCurve::FrequencyCurve (TaroMatchingAudioProcessor& eqProcessor) :
...
void FrequencyCurve::drawFrequencyCurve()
{
    ...
    frequencyCurvePath.clear();
    frequencyCurvePath.startNewSubPath (0.0f, juce::jmap(float(juce::Decibels::decibelsToGain(magnitudesOut[0]) + outputGain), -24.0f, 24.0f, 1.0f, 0.0f) * float(height));

    for (size_t i = 1; i < frequencies.size(); ++i)
    {
        float xx = (std::log(frequencies[i] / 20.0f) / std::log(2.0f)) / 10.0f * width;
        float gain = juce::Decibels::gainToDecibels (magnitudesOut [i]) + outputGain;
        float yy = juce::jmap(gain, -24.0f, 24.0f, 1.0f, 0.0f) * height;
